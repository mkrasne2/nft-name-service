...
contract Domains is ERC721URIStorage {
  ...
  constructor(string memory _tld) payable ERC721("Kween Name Service", "KWN") {
    owner = payable(msg.sender);
    tld = _tld;
  }

  event nameRegistered(string name, address registrar);

  function price(string calldata name) public pure returns(uint) {
    uint len = StringUtils.strlen(name);
    require(len > 2 && len < 13);
    if (len == 3) {
      return 5 * 10**17; // .5 MATIC = 5 000 000 000 000 000 000 (18 decimals). 
    } else if (len == 4) {
      return 3 * 10**17; // This is 0.3
    } else {
      return 1 * 10**17; // This is 0.1
    }
  }
 ...
 function register(string calldata name) public payable {
    ...
    string memory json = Base64.encode(
      abi.encodePacked(
        '{"name": "',
        _name,
        '", "description": "A domain on the Kween name service", "image": "data:image/svg+xml;base64,',
